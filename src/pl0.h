...
int charCnt;  // character count, 刚刚读的字符的下标（非下一位置）
long ll;      // line length
...
SymbolType ssym[256];     // list of special symbols(hashtable)
                          // declaration, statement, factor begin symbol set
SymbolType declbegsys, statbegsys, facbegsys;
...
typedef struct Record {
  ...
  long level;
  long addr;
  // 对于带参的函数名，该域指向第一个参数；对于形参，该域指向下一个参数（末参数该域为
  // NULL）
  struct Record *para_link;
  long para_num;
} Record;
// symbol table(0号位置作为 position() 中顺序查找的哨兵)
Record table[txmax + 1];
