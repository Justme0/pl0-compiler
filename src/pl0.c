...
long is_arr(TypeRecord const *const type) {
	...
	assert(type_table < type && type <= type_table + type_tx);	// 类型名未定义，外部应检测过
...
void free_para_link(Record *begin, Record *end) {
	for (; begin != end; ++begin) {
		...
		{
			Record *p = begin->para_link;	// p 指向第一个参数，begin 指向链表的头结点（头结点非动态生成）
...
DataType getsym() {
	...
	DataType sym_type = NULL;	// 当 sym 是 number 时区别整数与实数，其他符号则返回为 NULL
...
LABEL_INIT:
	...
	if (isalpha(ch)) { 	// identified or reserved
		...
		if (k>=kk) {
			kk = k;	// kk 是全局变量，初始是al（标识符最大长度），若执行到这里，说明此次标识符长度>=上次标识符长度
		} else {
			do {
				--kk;
				a[kk] = ' ';
...
void enter(char const *const sym_id, enum object k, DataType down_type) {	// enter object into table
	long i;
	...
	switch(k) {
		...
		case VARIABLE:
			// var a, b: real; 这种声明只有当 real 出现时才知道 a, b 的类型，所以类型、地址和 dx 在外面填写 TODO:
			strcpy(table[++tx].name,sym_id);
			table[tx].kind = k;
			table[tx].level = lev;
			table[tx].para_link = NULL;
		...
		case TAG:
			...
			type_table[type_tx].dope_vec.n =
				type_table[type_tx].dope_vec.upper_bound - type_table[type_tx].dope_vec.lower_bound;
			type_table[type_tx].size =
				type_table[type_tx].dope_vec.elem_type->size * type_table[type_tx].dope_vec.n;
			type_table[type_tx].level = lev;
			for (ptrcd = &type_table[type_tx], ptrcd->dope_vec.const_offset = ptrcd->dope_vec.lower_bound;
					is_arr(ptrcd->dope_vec.elem_type);
					ptrcd = ptrcd->dope_vec.elem_type)
			{
		...
		if (sym == ident) {
			...
			if (table == prcd) {
				...
				if (lparen == sym) {
					...
					gen(ppa, 0, prcd->para_num);	// 栈顶指针恢复到参数压栈前的位置（对应实参压栈）
					if (rparen == sym) {
						getsym();
					} else {
						error(33);
						...
						prcd != table)
				{
					switch(prcd->kind) {
						...
						case VARIABLE:
							if (is_arr(prcd->type)) {
								gen(lda,lev-prcd->level,prcd->addr - prcd->type->dope_vec.const_offset);
							} else {
			...
			factor_type = down_type;	// 在 factor() 外面 getsym() 得到的 sym(number) 的类型是 down_type
			if (g_num>amax) {
				error(31);
				g_num = 0;
			}
...
DataType term(SymbolType fsys, DataType down_type) {
	...
	factor_type1 = factor(fsys | times | slash | divsym | modsym | andsym, down_type);
	while (sym == times || sym == slash || sym == divsym || sym == modsym || sym == andsym) {
		long cx_temp = cx;
		if (sym == andsym) {
			gen(jp0, 0, 0);
		}
		...
		mulop = sym;
		sym_type = getsym();
		factor_type2 = factor(fsys | times | slash | divsym | modsym | andsym, sym_type);
...
void actual_para_l(SymbolType fsys, DataType down_type, Record const *prcd) {
	...
	for (;;) {
		...
		if (prcd->para_link == NULL) {
			...
			// gen(sto, lev - prcd->level, prcd->addr);	// callee 知道实参位置，用时去取即可
		}
		if (comma == sym) {
			getsym();
		} else {
...
BreakNode * statement(SymbolType fsys) {
	...
	if (sym == ident) {
		...
		if (ident_ref_type != NULL &&
				ident_ref_type != type_table &&
				prcd != NULL &&
				prcd != table)
		{
		...
		if (prcd->kind == FUNC) {
			...
			// 数组常量部分编译时算好
			gen(sta, lev - prcd->level, (prcd->addr - prcd->type->dope_vec.const_offset));
		} else {
		...
		if (sym != lparen) {
			...
			do {
				...
				// e.g. arr[3]: ident_ref_type(of arr[3]) is real, prcd->type (of arr) is Vector
		...
		switch (rcd.kind) {
			case CONSTANT:
				printf("%-2ld CONSTANT %s val = %ld type = %s\n",
						i, rcd.name, rcd.val, rcd.type->name);
				break;

			case VARIABLE:
				printf("%-2ld VARIABLE %s lev = %ld addr = %ld type = %s\n",
						i, rcd.name, rcd.level, rcd.addr, rcd.type->name);
				break;

			case PROC:
				printf("%-2ld PROC     %s lev = %ld addr = %ld para_num = %ld\n",
						i, rcd.name, rcd.level, rcd.addr, rcd.para_num);
				break;

			case FUNC:
				printf("%-2ld FUNC     %s lev = %ld addr = %ld ret_type = %s para_num = %ld\n",
...
void formal_para_l() {
	...
	for (;;) {
				...
				sym == booleansym)
		{
			...
			// 			++dx;	// 参数压栈是 caller 的责任，不占 callee 的空间
			getsym();
...
void func_dec(SymbolType fsys) {
	long tmp_tx = 0;		// save current table index before processing nested procedures
	long is_func = 0;
...
void block(SymbolType fsys, long tx0) {
	...
	do {
		...
		if (sym==varsym) {
			getsym();	// sym 应是 ident
			do {
				long tx1 = tx + 1;	// tx1 指向将要定义的变量在符号表中的位置，用于反填类型信息
				vardeclaration();
				while(sym==comma) {
					getsym();
					vardeclaration();
				...
				if (sym & (integersym | realsym | booleansym | ident)) {	// 应检查 ident 是否为数组类型 TODO:
					long i;
					for (i = tx1; i <= tx; ++i) {
						table[i].type = sym_to_type(sym);	// 类型最好加在 vardeclaration() 中，待重构
						table[i].addr = dx;
	...
	pbn = statement(fsys | semicolon | endsym);		// 每个后跟集都包含上层后跟集，以便补救
	if (NULL != pbn) {
...
int main(int argc, char **argv) {
	...
	//	sprintf(infilename, "C:\\Users\\Administrator\\Desktop\\pl0ext_doc\\test programs\\PreTest%ld.pl0", i);
	//	scanf("%s", infilename);
	//	printf("\n");
